...
initial begin
    ...
    assert(ccif.ramREN) else $display("IF0: ramREN not asserted for core0 iREN");
    assert(ccif.ramaddr == 32'h0000_1000) else $display("IF0: ramaddr mismatch for core0 iREN");
    while (ccif.ramstate != ACCESS) @(posedge clk);
    assert(ccif.iload[0] == ccif.ramload) else $display("IF0: iload[0] != ramload on ACCESS");
    assert(ccif.iwait[0] == 1'b0) else $display("IF0: iwait[0] not deasserted on ACCESS");
    ...
    ccif.cif0.iREN = 1; ccif.cif1.iREN = 1;@(posedge clk);
    while (ccif.ramstate != ACCESS) @(posedge clk);
    ...
    reset_dut();
    ccif.cif0.daddr = BLK; ccif.cif0.dREN = 1; ccif.cif0.cctrans = 0; @(posedge clk);
    ...
    // assert(ccif.ccwait == 2'b00) else $display("wrong1");
    // assert(ccif.ccsnoopaddr == ccif.cif0.daddr);
    ...
    ccif.cif1.daddr = BLK; ccif.cif1.dREN = 1; ccif.cif1.cctrans = 1; @(posedge clk);
    ...
    // assert(ccif.ccwait == 2'b10) else $display("wrong1");
    // assert(ccif.ccsnoopaddr == ccif.cif0.daddr);
