#------------------------------------------------------------------
# Unit Test MSI M -> I
# transition(s) :  BusRdX/BusWB 
#------------------------------------------------------------------

org 0x0000

li t0, 0x1000       //address
li t1, 0x1FE0       //sync variable (kinda)
...
//wait until core 1 finishes it's store, sync = 2 when c1 is done
wait_c1: 
    lw t4, 0(t1)    //sync value?
    li t5, 2        
    bne t4, t5, wait_c1     //loop until it is 2

    lw t6, 0(t0)    //we should now be I due to our M copy being invalidated
...
wait_c0:
    ...
    li t4, 0xBBBB2222       //new value
    sw t4, 0(t0)            //PrWr miss -> BusRdx -> M
                            //core 0 is now invalidated, m->i for core 0
