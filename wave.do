onerror {resume}
quietly WaveActivateNextPane {} 0
add wave -noupdate /control_unit_tb/cuif/inst
add wave -noupdate /control_unit_tb/cuif/imm
add wave -noupdate /control_unit_tb/cuif/opcode
add wave -noupdate /control_unit_tb/cuif/rs1
add wave -noupdate /control_unit_tb/cuif/rs2
add wave -noupdate /control_unit_tb/cuif/rd
add wave -noupdate /control_unit_tb/cuif/branchPCSrc
add wave -noupdate /control_unit_tb/cuif/jumpPCsrc
add wave -noupdate /control_unit_tb/cuif/MemtoReg
add wave -noupdate /control_unit_tb/cuif/RegWEN
add wave -noupdate /control_unit_tb/cuif/dWEN
add wave -noupdate /control_unit_tb/cuif/dREN
add wave -noupdate /control_unit_tb/cuif/iREN
add wave -noupdate /control_unit_tb/cuif/ALUSrc
add wave -noupdate /control_unit_tb/cuif/halt
add wave -noupdate /control_unit_tb/cuif/aluop
TreeUpdate [SetDefaultTree]
WaveRestoreCursors {{Cursor 1} {656 ns} 0}
quietly wave cursor active 1
configure wave -namecolwidth 150
configure wave -valuecolwidth 100
configure wave -justifyvalue left
configure wave -signalnamewidth 1
configure wave -snapdistance 10
configure wave -datasetprefix 0
configure wave -rowmargin 4
configure wave -childrowmargin 2
configure wave -gridoffset 0
configure wave -gridperiod 1
configure wave -griddelta 40
configure wave -timeline 0
configure wave -timelineunits ns
update
WaveRestoreZoom {0 ns} {1 us}
